...
| 600 | SemiBold | `.button`, `h3`–`h4`, `.text-link`, `.lead` |
...
Исключением является водяной знак на странице `page-style.php`, который интегрируется в качестве фона страницы. Повторное применение на данный момент не планируется.
...
|---|---|---|
| `.card:hover` | `border-color: var(--muted)` при наведении | Интерактивный элемент "карточка" показывает пользователю тот факт, что на него можно нажать.  |
| `.menu-toggle span` | `transform` при открытии меню | Пользователь должен понимать, что функционал кнопки "меню" изменился после нажатия. |
| `.toc-sidebar a` | `color` + `border-color` | Подсветка текущего раздела статьи. Функциональное выделение для упрощения пользовательского опыта. |
| `.swatch-chip:hover` | `translateX(±4px)` **без перехода** | Рывок, не анимация. Карточка цвета является функциональным элементом. Интерактивность позволяет пользователю лучше рассмотреть цвет. |
| `.swatch-chip--dark` | `box-shadow` **просто тень** | К сожалению, пришлось использовать тень чтобы элементы цветовой айдентики были видны. |
| `.swatch-chip--light` | `box-shadow` **просто тень** | Аналогично верхнему пункту. |
| `.zezh-lb-overlay` | `opacity` | Плавное появление просмотрщика фото необходимо для переключения внимания пользователя. Цель: снижение нагрузки на психику и зрение. |
...
Анимации и размытие используется **исключительно** в функциональных целях:
- Обозначение фокуса: если применение размытия снижает нагрузку на зрение и расслабляет психику, то его можно использовать
- Анимации раскрытия пунктов: допускаются только тогда, когда они визуально подтверждают изменение состояния интерфейса и помогают пользователю быстрее распознать результат своего действия.
- Анимации переключения состояния для визуальной идентификации интерактивных объектов: если необходимо прямо указать пользователю, что объект интерактивен, но использование типографических элементов обозначающих действие (←→↑↓↗↙) невозможно или недостаточно стоит использовать элементы выделения с использованием разрешённых методов (фон, толщина текста, обводка)
